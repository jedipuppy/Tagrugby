rugby_AI.AI0= function(pos,turn,select,ball,tagged){	//係数の設定
	//移動用
	let A = 20; //ゴールラインに対する前後の移動距離の係数
	let B = 5; //ディフェンスとの最短距離の係数
	let C = 5; //ボールを持っているプレイヤーとの縦の距離
	let D = -2; //ボールを持っているプレイヤーとの横の距離
	//パス用
	let G = 2 //パスの距離の係数
	let E = -3 //ボールを持っているプレイヤーとディフェンスとの最短距離の係数
	let F = 7 //パスを受けるプレイヤーとディフェンスとの最短距離の係数
	//変数の設定
	let forward_param;
	let eval_list = [];
	let action_list = [];
