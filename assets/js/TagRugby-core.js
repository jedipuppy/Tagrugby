...
rugby_AI.AttackSample1 = function (pos, turn, select, ball, tagged) {
	...
	best_move_array = [movablelist[best_move_index][0], movablelist[best_move_index][1]]; //評価値が最も大きい候補の動く場所を返す
...
rugby_AI.AttackSample2 = function (pos, turn, select, ball, tagged) {
	...
	for (let i = 0; i < movablelist.length; i++) {
		dis_defense = dis_defense_arr_func(pos, movablelist[i][0], movablelist[i][1]); //dis_defense（ディフェンスとの距離のリストを取得）
		if (check_func(dis_defense)) { //インターセプトを次のターンまでにされる場合
			eval_list.push(-100);
		} else { //インターセプトが次のターンに起こらない場合
			dis_defense_sum = sum(dis_defense);
			eval_list.push(-1 * forward_factor * (movablelist[i][1] - pos[1][select][1]) + dis_defense_sum + 0.1 * Math.random());
		}
	}
	best_move_index = eval_list.indexOf(Math.max.apply(null, eval_list)); //評価値が最も大きい候補の番号を得る
	best_move_array = [movablelist[best_move_index][0], movablelist[best_move_index][1]]; //評価値が最も大きい候補の動く場所を返す
...
rugby_AI.DefenseSample2 = function (pos, turn, select, ball, tagged) {
	...
	if (turn == -1) {
		for (let i = 0; i < movablelist.length; i++) {
			dis_ball = distance((movablelist[i][0] - pos[1][ball][0]), (movablelist[i][1] - pos[1][ball][1]));
...
rugby_AI.AttackSample3 = function (pos, turn, select, ball, tagged) {
	...
	if (select == ball) { //ボールを投げるか判断
		dis_attack = dis_attack_arr_func(pos, pos[1][select][0], pos[1][select][1]); //dis_attack（アタックとの距離のリストを取得）
		dis_defense_min = Math.min.apply(null, dis_defense_arr_func(pos, pos[1][select][0], pos[1][select][1])); //dis_defense（ディフェンスとの距離のリストを取得）
		for (let i = 0; i < pos[1].length; i++) {
			dis_defense_catch_min = Math.min.apply(null, dis_defense_arr_func(pos, pos[1][i][0], pos[1][i][1])); //キャッチするエージェントに最も近いディフェンスとの距離
			if ( //ボールを投げるかどうかの判断
				i != select &&
				dis_defense_min < pass_threshold &&
				dis_defense_catch_min - dis_defense_min > 1 &&
				pos[1][select][1] <= pos[1][i][1] &&
				Math.random() < catch_prob(pos[1][i][0], pos[1][i][1], pos[1][select][0], pos[1][select][1])
			) {
				return [
					[pos[1][i][0], pos[1][i][1]], eval_list
				];
			}
		}
	}
	...
	for (let i = 0; i < movablelist.length; i++) {
		...
		if (check_func(dis_defense)) { //インターセプトを次のターンまでにされる場合
			...
			if (ball == select) { //ボールを持っている場合はできるだけ前に行くようにする
				forward_param = -1 * (movablelist[i][1] - pos[1][select][1]);
			} else { //ボールを持っていない場合はボールを持っている人と同じラインに並ぼうとする。
				forward_param = (-1) * Math.abs(pos[1][ball][1] - movablelist[i][1]);
			}
			dis_defense_sum = sum(dis_defense);
			eval_list.push(forward_factor * forward_param + 0.1 * dis_defense_sum + 0.1 * Math.random());
		}
	}

	best_move_index = eval_list.indexOf(Math.max.apply(null, eval_list));
	best_move_array = [movablelist[best_move_index][0], movablelist[best_move_index][1]];
...
rugby_AI.DefenseSample3 = function (pos, turn, select, ball, tagged) {
	...
	for (let i = 0; i < movablelist.length; i++) {
		dis_ball = distance((movablelist[i][0] - pos[1][charge][0]), (movablelist[i][1] - pos[1][charge][1]));
		eval_list.push(-dis_ball + 0.1 * Math.random());
	}
...
rugby_AI.AttackSample4 = function (pos, turn, select, ball, tagged) {
	...
	dis_defense_min = Math.min.apply(null, dis_defense_arr_func(pos, pos[1][select][0], pos[1][select][1])); //dis_defense　ディフェンスとの最小距離
	if (select == ball) { //ボールを投げるか判断
		...
		for (let i = 0; i < pos[1].length; i++) {
			...
			if (
				...
				catch_eval_list.push(
					prob_factor * catch_prob(pos[1][i][0], pos[1][i][1], pos[1][select][0], pos[1][select][1]) +
					center_factor * Math.abs(pos[1][i][0] - BOARDSIZE / 2 +
		...
		if (catch_candidate.length != 0) {
			best_catch_index = catch_eval_list.indexOf(Math.max.apply(null, catch_eval_list));
			best_catch_array = [catch_candidate[best_catch_index][0], catch_candidate[best_catch_index][1]];
	...
	for (let i = 0; i < movablelist.length; i++) {
		...
		if (check_func(dis_defense)) { //インターセプトを次のターンまでにされる場合
			...
			if (ball == select) { //ボールを持っている場合はできるだけ前に行くようにする
				forward_param = -1 * (movablelist[i][1] - pos[1][select][1]);
				even_param = 0;
			} else { //ボールを持っていない場合はボールを持っている人とbase_lineだけ低いラインに並ぼうとする。
				if (movablelist[i][1] < pos[1][ball][1]) {
					forward_param = -10 * (pos[1][ball][1] - movablelist[i][1]);
				} else {
					forward_param = (-1) * Math.abs(pos[1][ball][1] - movablelist[i][1] + base_line) + 0.1 * (movablelist[i][1] - pos[1][ball][1]);
					even_param = -1 * Math.abs(attackLine[select] / (pos[1].length - 1) * BOARDSIZE - movablelist[i][0]);
				}

			}
			eval_list.push(forward_factor * forward_param + dis_defense_factor * dis_defense_min_next + even_factor * even_param + 0.1 * Math.random());
		}
	}
...
rugby_AI.AttackSample5 = function (pos, turn, select, ball, tagged) {
	...
	for (let i = 0; i < movablelist.length; i++) {
		...
		[forth_from_ball, back_from_ball] = backforthBallFunc(pos, ball, select, movablelist[i][1]);

		[defenseLine, attackLine] = PosSortTraverse(pos); //defenseLine,attackLineはそれぞれ左からエージェントのIDをリストにしたもの。
		deviation_from_uniform_position = -Math.abs(attackLine[select] / (pos[1].length - 1) * BOARDSIZE - movablelist[i][0]); //均等に横方向に並ぶ場合の場所からのずれ
		eval_list.push(
			...
			deviation_from_uniform_position_factor * deviation_from_uniform_position +
			0.1 * Math.random()
		);
	}
	...
	for (let i = 0; i < passlist.length; i++) {
		...
		catch_probability = catch_prob(pos[1][i][0], pos[1][i][1], pos[1][select][0], pos[1][select][1]); //パスが成功する確率
...
class Game {
	...
	humanTurn(x, y) {
		...
		if (this.select == this.wait) {
			this.wait = -1;
			[this.select, this.turn, this.tagged] = stepPhaseFunc(this.select, this.pos, this.turn, this.ball, this.tagged);
		}
		//動ける場所がない場合の処理
		...
		//タグが取られた場合の処理
			...
			if (this.tag == 1) {
				...
				[this.select, this.turn, this.tagged] = taggedstepPhaseFunc(this.select, this.pos, this.turn, this.ball, this.tagged);
			}
		} else {

			let catchable = catchableFunc(this.pos, this.turn, this.select, this.ball, x, y);
			//パスした場合の処理
			if (catchable != -1) {
				this.wait = try_catch(this.ball, catchable, x, y, this.pos[this.turn][this.select][0], this.pos[this.turn][this.select][1], this.wait);
				this.ball = catchable;
				[this.select, this.turn, this.tagged] = stepPhaseFunc(this.select, this.pos, this.turn, this.ball, this.tagged);
			}
			//移動する場合の処理
	...
	AIturn() { //AIの思考
		...
		//タグが取られた場合の処理
		else if (tagJudgeFunc(this.select, this.pos, this.turn, this.ball, nextmove[0], nextmove[1])) {
			if (this.tag == 1) {
			...
			let catchable = catchableFunc(this.pos, this.turn, this.select, this.ball, nextmove[0], nextmove[1]);
			//パスした場合の処理
			if (catchable != -1) {
				this.wait = try_catch(this.ball, catchable, x, y, this.pos[this.turn][this.select][0], this.pos[this.turn][this.select][1], this.wait);
				this.ball = catchable;
...
function tagJudgeFunc(select, pos, turn, ball, x, y) {
	if (Math.abs(pos[arrayTurn(turn)][select][0] - x) <= 1 && Math.abs(pos[arrayTurn(turn)][select][1] - y) <= 1 && ([x, y] != pos[arrayTurn(turn)][select])) {
...
function stepPhaseFunc(select, pos, turn, ball, tagged) {
	...
	if (turn == -1) {
		if (select == pos[arrayTurn(turn)].length - 1) {
			select = 0;
		} else {
			select += 1;
		}
...
function movablelistFunc(pos, turn, select, tagged) {
	...
	for (let i = -1; i <= 1; i++) {
		overlapSpace: for (let j = -1; j <= 1; j++) {
			if (!(i == 0 && j == 0) && pos[arrayTurn(turn)][select][0] + i >= 0 && pos[arrayTurn(turn)][select][0] + i < BOARDSIZE && pos[arrayTurn(turn)][select][1] + j >= 0 && pos[arrayTurn(turn)][select][1] + j < BOARDSIZE) {
				for (let k = 0; k < pos[0].length; k++) {
					if (pos[arrayTurn(turn)][select][0] + i == pos[0][k][0] && pos[arrayTurn(turn)][select][1] + j == pos[0][k][1]) {
						continue overlapSpace;
					}
				}
				for (let k = 0; k < pos[1].length; k++) {
					if (pos[arrayTurn(turn)][select][0] + i == pos[1][k][0] && pos[arrayTurn(turn)][select][1] + j == pos[1][k][1]) {
						continue overlapSpace;
					}
				}
				movablelist.push([pos[arrayTurn(turn)][select][0] + i, pos[arrayTurn(turn)][select][1] + j]);
			}
		}
	}
	return movablelist;
	...
	if (select == ball) { //ボールを投げるか判断
		...
		for (let i = 0; i < pos[1].length; i++) {
			if (
				...
				i != select &&
				pos[1][select][1] <= pos[1][i][1] &&
				distance(pos[1][select][0] - pos[1][i][0], pos[1][select][1] - pos[1][i][1]) <= MAX_PASS_LENGTH
			) {
...
function CheckMateFunc(pos, turn, select, ball) {
	if (turn == -1) {
		for (let i = -1; i <= 1; i++) {
			for (let j = -1; j <= 1; j++) {
				if (ball, pos[arrayTurn(turn)][select][0] + i == pos[1][ball][0] && pos[arrayTurn(turn)][select][1] + j == pos[1][ball][1]) {
					return [1, pos[arrayTurn(turn)][select][0] + i, pos[arrayTurn(turn)][select][1] + j];
				}
			}
		}
	} else if (turn == 1) {
...
function catchableFunc(pos, turn, select, ball, x, y) {
	if (turn == 1 && select == ball) {
		for (let i = 0; i < pos[1].length; i++) {
			if (pos[1][i][0] == x && pos[1][i][1] == y && pos[1][i][1] >= pos[1][select][1] && i != select) {
				return i;
			}
		}
	}
...
function gameOver(result_Diff, x, y) {
	...
	//勝敗の判定
	if (result_Diff == 1) { //アタック勝ち
		attack_win_num = parseInt(document.getElementById('attack_win_num').innerHTML);
		document.getElementById('attack_win_num').innerHTML = attack_win_num + 1;
		document.getElementById('result').innerHTML = 'アタックの勝ち！';
	} else if (result_Diff == -1) { //ディフェンス勝ち
		defense_win_num = parseInt(document.getElementById('defense_win_num').innerHTML);
...
function draw(ctx, canvas) {
	...
	ctx.fillRect(game.pos[arrayTurn(game.turn)][game.select][0] * BLOCKSIZE + NUMSIZE + 0.5, game.pos[arrayTurn(game.turn)][game.select][1] * BLOCKSIZE + NUMSIZE + 0.5, BLOCKSIZE, BLOCKSIZE);
	ctx.fill();
	...
	for (let i = 0; i <= 1; i++) {
		...
		for (let j = 0; j < game.pos[i].length; j++) {
			ctx.beginPath();
			ctx.arc(game.pos[i][j][0] * BLOCKSIZE + ~~(BLOCKSIZE * 0.5) + NUMSIZE + 0.5, game.pos[i][j][1] * BLOCKSIZE + ~~(BLOCKSIZE * 0.5) + NUMSIZE + 0.5, BLOCKSIZE / 2 * 0.8, 0, 2 * Math.PI, false);
			ctx.fill();
			ctx.stroke();
		}
	}
	...
	if (game.select != game.wait) {
		ctx.globalAlpha = 0.2;
		let movablelist = movablelistFunc(game.pos, game.turn, game.select, this.tagged);
		if (game.turn == -1) {
			ctx.fillStyle = DEFENSEFILLCOLOR;
			ctx.strokeStyle = DEFENSEBORDERCOLOR;
		}
		...
		for (let i = 0; i < movablelist.length; i++) {
			ctx.beginPath();
			ctx.arc(movablelist[i][0] * BLOCKSIZE + ~~(BLOCKSIZE * 0.5) + NUMSIZE + 0.5, movablelist[i][1] * BLOCKSIZE + ~~(BLOCKSIZE * 0.5) + NUMSIZE + 0.5, BLOCKSIZE / 2 * 0.8, 0, 2 * Math.PI, false);
			ctx.fill();
			ctx.stroke();
		}
		if (game.select == game.ball) {
			let passlist = passlistFunc(game.pos.copy(), game.turn, game.select, game.ball);
			ctx.fillStyle = BALLCOLOR;
			ctx.globalAlpha = 0.6;
			for (let i = 0; i < passlist.length; i++) {
				ctx.beginPath();
				ctx.arc(passlist[i][0] * BLOCKSIZE + ~~(BLOCKSIZE * 0.5) + NUMSIZE + 0.5, passlist[i][1] * BLOCKSIZE + ~~(BLOCKSIZE * 0.5) + NUMSIZE + 0.5, BLOCKSIZE / 2 * 0.3, 0, 2 * Math.PI, false);
				ctx.fill();
				ctx.stroke();
			}
		}
...
function analysis() {
	...
	action_list = [];
	let movablelist = movablelistFunc(game.pos.copy(), game.turn, game.select, game.tagged); //動ける場所のリストを作成
	let passlist = passlistFunc(game.pos.copy(), game.turn, game.select, game.ball); //動ける場所のリストを作成
	action_list = action_list.concat(movablelist);
	action_list = action_list.concat(passlist);
	...
	Role[2] = document.ControlForm.ana_role.value;
	AI_name = Role[2];
	console.log(rugby_AI[AI_name](game.pos, game.turn, game.select, game.ball, game.tagged));
	[nextmove, eval_list] = rugby_AI[AI_name](game.pos, game.turn, game.select, game.ball, game.tagged);
	ctx.fillStyle = ANAMOVEFONTCOLOR;
	for (let i = 0; i < movablelist.length; i++) {
		ctx.fillText(Math.floor(eval_list[i]), action_list[i][0] * BLOCKSIZE + ~~(BLOCKSIZE * 0.5) + NUMSIZE + 0.5, action_list[i][1] * BLOCKSIZE + ~~(BLOCKSIZE * 0.5) + NUMSIZE + 0.5);
	}
...
function upload() {
	try {
		movablelist_temp = movablelistFunc(game.pos.copy(), game.turn, game.select, game.tagged); //動ける場所のリストを作成
		passlist_temp = passlistFunc(game.pos.copy(), game.turn, game.select, game.ball); //動ける場所のリストを作成
